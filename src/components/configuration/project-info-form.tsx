...
interface ProjectInfoFormProps {
	project: Project
	onSave: (patch: Partial<Omit<Project, "id">>) => Promise<void>
}

export function ProjectInfoForm({ project, onSave }: ProjectInfoFormProps) {
	const [name, setName] = useState(project.name)
	const [domain, setDomain] = useState(project.domain)
	const [country, setCountry] = useState(project.country)
	const [countryCustom, setCountryCustom] = useState(project.country_custom ?? "")
	const [language, setLanguage] = useState(project.language)
	const [languageCustom, setLanguageCustom] = useState(project.language_custom ?? "")
	...
	useEffect(() => {
		...
		setCountryCustom(project.country_custom ?? "")
		setLanguage(project.language)
		setLanguageCustom(project.language_custom ?? "")
	}, [project])
	...
	const handleSave = async () => {
		...
		try {
			...
			if (country === "other" && countryCustom.trim()) {
				patch.country = countryCustom.trim().toUpperCase().slice(0, 2)
			}
			if (language === "other" && languageCustom.trim()) {
				patch.language = languageCustom.trim().toLowerCase().slice(0, 2)
			}
			await onSave(patch)
			setSaved(true)
