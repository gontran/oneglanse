	...
	for (const prompt of PROMPTS) {
		for (const surface of SURFACES) {
			...
			const position = mentioned
				? Math.max(1, Math.round(prog.avgPosition + (hashStr(`${run.id}-${surface}`) % 3) - 1))
				: null
			const sentiment = mentioned
				? clamp(prog.sentiment + (hashStr(`${run.id}-${surface}-sent`) % 12) - 6, 0, 100)
				: 0
			const recScore = mentioned
				? clamp(prog.recommendationScore + (hashStr(`${run.id}-${surface}-rec`) % 14) - 7, 0, 100)
				: 0
			const visScore = mentioned
				? clamp(prog.visibilityScore + (hashStr(`${run.id}-${surface}-vis`) % 14) - 7, 0, 100)
				: 0
			const recType =
				recScore >= 70
					? "top_pick"
			...
			if (prompt.id === "prompt-5" || prompt.id === "prompt-6")
				sources.push({
			...
			const risks = isLatest
				? []
				: run.id === "run-j29"
					? ["faible visibilite dans les reponses IA"]
					: []
			const coreClaims = [
				"catalogue de films et series en VOD legale",
