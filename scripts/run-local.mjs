...
async function main() {
	...
	const stopPackageWatchers = packageWatchers.map((child) => attachTerminationHandler(child))
	...
	await Promise.all([
		...packageWatchers.map((child, index) =>
			waitForChildExit(child, `Workspace package watch ${LOCAL_WATCH_PACKAGES[index]}`),
		),
