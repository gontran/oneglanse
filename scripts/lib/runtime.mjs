...
const CAMOUFOX_PYTHON_CANDIDATES = ["python3.12", "python3.11", "python3.10", "python3"]
...
function buildRootEnvTemplate(rawTemplate) {
	return rawTemplate
		...
		.replace(/^INTERNAL_CRON_SECRET=.*$/m, `INTERNAL_CRON_SECRET=${randomUUID()}`)
}
...
function ensureGeneratedSecrets(rawEnv) {
	const replacements = [
		{
			...
			shouldReplace: (value) => value === "" || value === "replace-me" || value === "changeme",
	...
	for (const replacement of replacements) {
		...
		if (replacement.shouldReplace(currentValue)) {
			nextEnv = nextEnv.replace(pattern, `${replacement.key}=${replacement.buildValue()}`)
			changed = true
		}
	}
...
export async function terminateLocalProcesses(commandFragments) {
	...
	const { stdout } = await runCommandCapture("ps", ["ax", "-o", "pid=", "-o", "command="], {
...
function killChildProcessTree(child, signal = "SIGTERM") {
	...
	if (process.platform === "win32") {
		...
		void runCommandCapture("taskkill", ["/pid", String(pid), "/t", ...forceFlag], {
...
export function buildLocalRuntimeEnv(localAppUrl) {
	...
	const localLocale =
		process.env.CAMOUFOX_LOCALE || Intl.DateTimeFormat().resolvedOptions().locale || "en-US"
	const localEnv = {
		...process.env,
...
export function spawnLocalWorkspacePackageWatchers(env) {
	return LOCAL_WATCH_PACKAGES.map((pkg) =>
		spawnCommand("pnpm", ["--filter", pkg, "exec", "tsc", "--watch", "--preserveWatchOutput"], {
			env,
		}),
	)
}
...
async function getPythonVersion(command) {
	try {
		...
		if (typeof parsed?.major === "number" && typeof parsed?.minor === "number") {
			return parsed
		}
	} catch {}
...
async function installCompatiblePython() {
	...
	if (process.platform === "win32" && (await canRunCommand("winget", ["--info"]))) {
		console.log("Installing Python 3.11 for local Camoufox support...")
		await runCommand("winget", ["install", "-e", "--id", "Python.Python.3.11", "--silent"])
		return "python"
	}
...
async function resolveLocalCamoufoxPython() {
	...
	const candidates = [...(configured ? [configured] : []), ...CAMOUFOX_PYTHON_CANDIDATES]
	...
	throw new Error("Python 3.10+ is still unavailable after attempting automatic installation.")
}
...
async function ensureCamoufoxPackage(pythonBin) {
	...
	installArgs.push(process.env.CAMOUFOX_PIP_SPEC?.trim() || CAMOUFOX_DEFAULT_PIP_SPEC)
	await runCommand(pythonBin, installArgs)
}

async function ensureCamoufoxBrowser(pythonBin) {
	const desiredChannel =
		process.env.CAMOUFOX_BROWSER_CHANNEL?.trim() || CAMOUFOX_DEFAULT_BROWSER_CHANNEL

	let activeChannel = null
	let browserInstalled = false

	try {
		const [{ stdout: activeStdout }, { stdout: versionStdout }] = await Promise.all([
