...
function getSessionFile(provider) {
	return path.join(getAuthRootDir(), "sessions", provider, `${provider}-auth.json`)
}
...
async function uploadExistingSessionsIfPresent(uploadUrl, uploadToken, providers = null) {
	const uploaded = []
	...
	for (const [index, { provider, sessionFile }] of sessionFiles.entries()) {
		const rawSession = await readFile(sessionFile)
		const prefix = Buffer.from(`{"provider":${JSON.stringify(provider)},"session":`)
...
async function main() {
	...
	try {
		...
		console.log("Finish provider sign-in in the browser, then return here and press Enter.")
		await prompt("Press Enter when you are done connecting providers. ")

		const sessionSnapshotAfter = await captureSessionSnapshot()
		const changedProviders = getChangedProviders(sessionSnapshotBefore, sessionSnapshotAfter)

		if (changedProviders.length === 0) {
			console.log("No new or updated provider sessions were detected.")
			if (uploadUrl && uploadToken) {
				console.log("Skipping upload because no local provider sessions changed during this run.")
			}
			return
		}

		console.log(`Saved or updated provider sessions: ${changedProviders.join(", ")}`)
		...
		if (uploadAnswer === "y" || uploadAnswer === "yes") {
			await uploadExistingSessionsIfPresent(uploadUrl, uploadToken, changedProviders)
			return
		}
