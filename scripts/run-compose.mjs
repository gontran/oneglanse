...
const ARCH_MISMATCH_PATTERNS = ["no matching manifest", "no match for platform in manifest"]
...
function isArchitectureMismatchError(error) {
	...
	return ARCH_MISMATCH_PATTERNS.some((pattern) => message.toLowerCase().includes(pattern))
}
...
async function runSmartPull() {
	...
	if (pullableServices.length === 0) {
		console.log("Skipping docker pull: no remote images configured for the app stack.")
		return
	}
