	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		try {
			...
			if (
				...
				logger.warn(`refreshing ${provider} page before retry due to ${failureType}`)
				await config.beforeRetryHook(page)
			}

			if (useProxy && !proxyProven && shouldRotateImmediatelyOnUnprovenProxy(failureType)) {
				logger.warn(`canary failed on unproven proxy with ${failureType} — rotating IP immediately`)
			...
			if (attempt === maxAttempts) {
				if (!useProxy) {
					logger.error(`prompt ${promptIndex + 1} exhausted ${maxAttempts} attempts`)
					throw err
				}
				logger.error(
