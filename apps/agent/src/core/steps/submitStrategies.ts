...
import { clickLocatorLikeUser, randomBetween } from "../../lib/browser/humanBehavior.js"
...
async function ensureInputHasWords(ctx: SubmitContext, attemptLabel: string): Promise<boolean> {
	...
	const content = normalizedLiveContent.length > 0 ? liveContent : ctx.preSubmitContent
	...
	throw new ExternalServiceError(ctx.provider, `${EMPTY_INPUT_SUBMIT_ERROR} (${attemptLabel})`)
}
