...
import { insertPromptIntoEditor, normalizePromptValue } from "../../lib/input/editor/promptInput.js"
...
export async function askPrompt(page: Page, prompt: string, provider: Provider): Promise<void> {
	...
	const preSubmitContent = await input.readInputValue().catch(() => insertedValue)
	const preSubmitUrl = page.url()

	if (!preSubmitContent || normalizePromptValue(preSubmitContent).length === 0) {
