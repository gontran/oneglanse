...
async function isEditorReady(input: Locator, provider: Provider): Promise<boolean> {
...
export async function waitForEditorReady(page: Page, provider: Provider): Promise<Locator> {
	...
	const primaryGraceMs = PRIMARY_SELECTOR_GRACE_MS[provider] ?? DEFAULT_PRIMARY_SELECTOR_GRACE_MS
	let polls = 0
		...
		const input =
			primarySelector && elapsedMs < primaryGraceMs
				? await waitForStableEditorCandidate(page, provider, () =>
						findActiveEditorCandidateFromSelectors(page, [primarySelector]).catch(() => null),
					)
				: await waitForStableEditorCandidate(page, provider, () =>
