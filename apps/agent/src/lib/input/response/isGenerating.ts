...
export async function getGenerationStateSignature(page: Page, provider: Provider): Promise<string> {
...
export async function getResponseStateSignature(
	...
	return await page.evaluate((selectors) => {
		const visible = (element: Element | null): element is HTMLElement => {
			...
			const style = window.getComputedStyle(element)
			return (
				element.offsetParent !== null && style.visibility !== "hidden" && style.display !== "none"
			)
		}

		const elements = (selectors || [])
			.flatMap((selector) => Array.from(document.querySelectorAll(selector)))
			.filter((el): el is HTMLElement => visible(el) && (el.innerText || "").trim().length > 0)
