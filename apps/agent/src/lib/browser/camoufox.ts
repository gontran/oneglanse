...
function resolveHeadlessMode(
	...
	return (
		override ??
		(process.env.CAMOUFOX_HEADLESS_MODE as "virtual" | "headful" | "headless" | undefined) ??
		DEFAULT_CAMOUFOX_HEADLESS_MODE
	)
}

function getHeadlessValue(override?: "virtual" | "headful" | "headless"): boolean {
...
async function resolvePythonBinary(provider: Provider): Promise<string> {
	...
	const candidates = [process.env.CAMOUFOX_PYTHON_BIN?.trim(), ...PYTHON_CANDIDATES].filter(
...
function parseStringOrList(name: string, raw: string | undefined): string | string[] | undefined {
	const parsed = parseStringList(name, raw)
	if (!parsed || parsed.length === 0) return undefined
	return parsed.length === 1 ? parsed[0] : parsed
}

function parseStringPair(name: string, raw: string | undefined): [string, string] | undefined {
	if (!raw) return undefined
...
function parsePositiveInteger(name: string, raw: string | undefined): number | undefined {
	if (!raw) return undefined
...
function parseFingerprintPreset(raw: string | undefined): boolean | JsonRecord | undefined {
	if (!raw) return undefined
...
async function listFontsWithPowerShell(): Promise<string[]> {
	...
	return dedupeStrings(
		parsed
			.map((value) => (typeof value === "string" ? normalizeFontFamilyName(value) : null))
			.filter((value): value is string => Boolean(value)),
	)
}
...
function pickBrowserEnv(args?: {
	...
	return {
		...baseEnv,
		...(parsePrimitiveRecord("CAMOUFOX_ENV_JSON", process.env.CAMOUFOX_ENV_JSON) ?? {}),
	}
}
...
async function buildLaunchPayload(args: {
	...
	const extraLaunch = args.plainAuthMode
		...
		: args.disableFingerprinting
			? {}
			: (parseJsonRecord("CAMOUFOX_EXTRA_LAUNCH_JSON", process.env.CAMOUFOX_EXTRA_LAUNCH_JSON) ??
				{})

	const extraArgs =
		Array.isArray(extraLaunch.args) && extraLaunch.args.every((item) => typeof item === "string")
	...
	const config = {
		...(args.plainAuthMode
			...
			: {
					...
					...(parseJsonRecord("CAMOUFOX_CONFIG_JSON", process.env.CAMOUFOX_CONFIG_JSON) ?? {}),
				}),
		showcursor: false,
	...
	const locale = args.plainAuthMode
		? "en-US"
		: (parseStringOrList("CAMOUFOX_LOCALE", process.env.CAMOUFOX_LOCALE) ?? "en-US")
	...
	const excludeAddons = args.plainAuthMode
		? undefined
		: parseStringList("CAMOUFOX_EXCLUDE_ADDONS", process.env.CAMOUFOX_EXCLUDE_ADDONS)?.map(
	...
	const webglConfig = args.plainAuthMode
		? undefined
		: parseStringPair("CAMOUFOX_WEBGL_CONFIG", process.env.CAMOUFOX_WEBGL_CONFIG)
	if (webglConfig !== undefined) payload.webgl_config = webglConfig

	const ffVersion = args.plainAuthMode
		? undefined
		: parsePositiveInteger("CAMOUFOX_FF_VERSION", process.env.CAMOUFOX_FF_VERSION)
	...
	if (args.plainAuthMode || args.disableFingerprinting) {
		...
		const fingerprint =
			parseJsonRecord("CAMOUFOX_FINGERPRINT_JSON", process.env.CAMOUFOX_FINGERPRINT_JSON) ??
		...
		const fingerprintPreset = parseFingerprintPreset(process.env.CAMOUFOX_FINGERPRINT_PRESET)
	...
	const argsList = args.plainAuthMode
		? undefined
		: (parseStringList("CAMOUFOX_ARGS", process.env.CAMOUFOX_ARGS) ?? extraArgs)
...
export async function resolveCamoufoxLaunchOptions(args: {
	...
	try {
		...
		const jsonLine =
			[...lines].reverse().find((l) => l.trimStart().startsWith("{")) ?? stdout.trim()
