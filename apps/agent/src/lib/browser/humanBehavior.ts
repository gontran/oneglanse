...
export async function getBrowserPrimaryModifier(page: Page): Promise<"Meta" | "Control"> {
	try {
...
export async function moveMouseToElement(page: Page, target: Locator): Promise<void> {
...
export async function preInteractionIdle(page: Page): Promise<void> {
	await page.waitForTimeout(CAMOUFOX_HUMANIZE ? randomBetween(80, 180) : randomBetween(300, 700))
}
