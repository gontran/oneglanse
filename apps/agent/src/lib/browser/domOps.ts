...
export async function runPageDomOp<T>(
	...
	return (await page.evaluate(
		({ operation: currentOperation, params: currentParams }) => {
			...
			function resolveSelectorWithin(root: ParentNode, selector: string): Element[] {
			...
			function isResponsePlaceholder(element: Element): boolean {
				return (
					...
					(element.getAttribute("data-message-id") || "").startsWith("request-placeholder")
				)
			}
			...
			function runProviderRawSourcesExtractor(extractorSource: string, selectors: string[]) {
			...
			function formatCitationAnchors(root: HTMLElement): void {
				const cleanCitationText = (text: string) => text.replace(/\+\d+$/, "").trim()
				...
				for (const anchor of Array.from(root.querySelectorAll("a[href]"))) {
					...
					anchor.replaceWith(document.createTextNode(" "), strong, document.createTextNode(" "))
				}
			}
			...
			function readResponseHtml(provider: string, selectors: string[]): string {
				...
				if (provider === "ai-overview") {
					...
					clone
						.querySelectorAll('[data-src-id], button, [role="button"], svg, img, style, script')
			...
			function detectBotPageState(): {
				...
				const signals: Array<{ matched: boolean; reason: string }> = [
					{
						matched:
							/sorry/i.test(url) || /our systems have detected unusual traffic/i.test(bodyText),
