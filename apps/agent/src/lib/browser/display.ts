...
function parseScreenSpec(): { width: number; height: number; depth: number } {
	...
	if (!match) {
		throw new Error("CAMOUFOX_XVFB_SCREEN must use WIDTHxHEIGHTxDEPTH, for example 1920x1080x24.")
	}
...
async function waitForDisplaySocket(
	...
	child.kill("SIGTERM")
	throw new Error(`Xvfb display :${displayNumber} was not ready within ${timeoutMs}ms`)
}
...
export async function ensureDisplay(options?: {
	...
	if ((process.env.CAMOUFOX_HEADLESS_MODE ?? DEFAULT_CAMOUFOX_HEADLESS_MODE) === "headful") {
		throw new Error("CAMOUFOX_HEADLESS_MODE=headful requires DISPLAY to already be set.")
	}
	...
	for (let attempt = 0; attempt < 5; attempt += 1) {
		...
		if (!Number.isInteger(displayNumber)) {
			throw new Error(`Invalid CAMOUFOX_XVFB_DISPLAY value "${display}". Expected format like :99.`)
		}
		const xvfb = spawn(
			xvfbBinary,
			[
