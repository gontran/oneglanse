	...
	for (let attempt = 0; attempt < ATTEMPTS_PER_CYCLE; attempt++) {
		...
		try {
			...
			if (failureType === "browser_crash") {
				plog.warn(`browser crash on attempt ${totalAttempt}/${totalMax}; retrying immediately`)
				continue
			}
...
export async function runWithRetryCycles(
	...
	for (let cycle = 0; cycle < MAX_CYCLES; cycle++) {
		if (cycle > 0) {
			const backoff = jitter(exponentialBackoff(cycle - 1, INITIAL_BACKOFF, MAX_CYCLE_BACKOFF))
			plog.warn(`cycle ${cycle + 1}/${MAX_CYCLES}: backing off ${backoff / 1000}s before retry...`)
			await sleep(backoff)
		}
	...
	plog.error(`exhausted — failed all ${totalAttempts} attempts across ${MAX_CYCLES} cycles`)
