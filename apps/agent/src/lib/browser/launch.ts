...
import { type Provider, resolveAppMode, shouldUseProxyInMode } from "@oneglanse/types"
import { logger } from "@oneglanse/utils"
import type { Browser, BrowserContext } from "playwright"
import { firefox } from "playwright-core"
import { env } from "../../env.js"
import { type CamoufoxProxyConfig, resolveCamoufoxLaunchOptions } from "./camoufox.js"
...
function formatProxyServerUrl(scheme: ProxyScheme, host: string, port: number): string {
...
function parseThorDataProxyLine(value: string): { host: string; port: number } | null {
...
async function acquireThorDataProxyInner(): Promise<ProxyAllocation> {
	...
	if (!apiUrl) {
		throw new Error("THORDATA_PROXY_API_URL is required when using ThorData API proxy discovery.")
	}
...
function toCamoufoxProxyConfig(proxy: UpstreamProxyConfig | null): CamoufoxProxyConfig | undefined {
...
export async function launchContext(provider: Provider): Promise<{
	...
	try {
		...
		const display = runtimeHeadlessMode === "headless" ? undefined : displayHandle?.display
		...
		rawContext = await rawBrowser.newContext({
			...
			...(runtimeSeedPlan.authStatePath ? { storageState: runtimeSeedPlan.authStatePath } : {}),
		})
		...
		await cleanup()
		throw new ExternalServiceError("browser", toErrorMessage(error), 502, { provider }, error)
	}
}
