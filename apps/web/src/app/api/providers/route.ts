...
import { resetProviderAuthData, spawnProviderAuthLogin } from "@oneglanse/services"
...
export async function POST(request: Request) {
	...
	if (!isLocalProvidersMode()) {
		return NextResponse.json(
			{
				error: "Interactive provider connect and refresh are only available in local mode.",
			},
			{ status: 405 },
		)
	}
...
export async function DELETE() {
	...
	try {
		await Promise.all(AUTH_PROVIDER_LIST.map((authProvider) => resetProviderAuthData(authProvider)))
		return NextResponse.json({ ok: true })
	} catch (error) {
		return NextResponse.json(
			{
				error: error instanceof Error ? error.message : "Failed to reset providers",
			},
			{ status: 500 },
		)
	}
