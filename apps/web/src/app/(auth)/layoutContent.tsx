	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<button
					type="button"
					className={cn(formToolbarButtonClassName, "flex items-center gap-2 px-4")}
				>
					<User2 className="h-4 w-4 shrink-0" />
					<span className="max-w-[140px] truncate">{userName || userEmail || "Account"}</span>
...
export default function LayoutContent({
	...
	const shouldFetchWorkspace = !!workspaceIdFromUrl && workspace?.id !== workspaceIdFromUrl
	...
	const shouldEnforceProviderGate = !resolvedWorkspace && !isResolvingWorkspaceFromUrl
	...
	const currentPathWithQuery = pathname
		...
		: "/workspace"
	const providerGateNext = rawNext ?? (isProvidersPage ? null : currentPathWithQuery)
	...
	useEffect(() => {
		...
		window.addEventListener(SKIP_PROVIDER_GATE_EVENT, handleSkipProviderGateChange)

		return () => {
			window.removeEventListener(SKIP_PROVIDER_GATE_EVENT, handleSkipProviderGateChange)
		}
	}, [])
		...
		return (
			<>
				...
				<main className="mx-auto flex min-h-svh w-full max-w-6xl flex-col px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
					...
					<ProvidersScreen
						title={canLaunchProvidersLocally ? "Connect Providers" : "Providers are required"}
						description={
