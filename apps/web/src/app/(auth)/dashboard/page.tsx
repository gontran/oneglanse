...
export default function Dashboard() {
	...
	const timeFilter = (searchParams.get("time") ?? "all") as "all" | "7d" | "14d" | "30d"
	...
	const hasAnyAnalysisInWorkspace = useMemo(() => {
		return analysedPromptData?.some((r) => Boolean(r?.is_analysed && r?.brand_analysis))
	}, [analysedPromptData])
	...
	const promptGroups = useMemo((): PromptGroup[] => {
		...
		const groupMap = new Map<string, { promptText: string; rows: AnalysisRecord[] }>()
	...
	if (!hasAnyAnalysisInWorkspace) return <NoAnalysisState workspaceId={workspaceId} />
		...
		<div className="web-page-wide">
			<div className="web-page-wide-inner">
				<div className="ui-stagger space-y-5 sm:space-y-6">
					...
					{!hasFilteredAnalysis ? (
						<FilteredDashboardState workspaceId={workspaceId} modelFilter={modelFilter} />
					) : (
						<>
							<AggregateStatsRow
