...
export function writeSkipProviderGate(value: boolean): void {
	...
	if (value) {
		...
		window.dispatchEvent(new CustomEvent(SKIP_PROVIDER_GATE_EVENT, { detail: { value: true } }))
		return
	}

	window.sessionStorage.removeItem(SKIP_PROVIDER_GATE_STORAGE_KEY)
	window.dispatchEvent(new CustomEvent(SKIP_PROVIDER_GATE_EVENT, { detail: { value: false } }))
}
