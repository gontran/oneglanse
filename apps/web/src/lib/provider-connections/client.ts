...
import type { ProviderConnectionRequest, ProviderConnectionsState } from "./types"
...
export function useProviderConnections(options?: {
	...
	return useQuery({
		...
		refetchInterval: (query) => {
			...
			const shouldWatchForExternalUpdates = options?.watchForExternalUpdates === true
...
export function useResetAllProviders(
	options?: Omit<UseMutationOptions<{ ok: boolean }, Error, void>, "mutationFn">,
) {
	const queryClient = useQueryClient()
	const { onSettled, ...restOptions } = options ?? {}
