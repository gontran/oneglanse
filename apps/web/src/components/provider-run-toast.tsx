...
export function persistActiveProviderRun(args: ActiveProviderRun): void {
	...
	window.localStorage.setItem(ACTIVE_PROVIDER_RUN_STORAGE_KEY, JSON.stringify(args))
...
function useProviderRunToast(args: {
	...
	const [stoppingProvider, setStoppingProvider] = useState<Provider | null>(null)
		...
		(provider: Provider) => {
			return async () => {
				...
				try {
					...
					setStoppingProvider((current) => (current === provider ? null : current))
				}
			}
		},
	...
	useEffect(() => {
		...
		if (transitionedProvider) {
			...
			const handoffDelay =
				nextPhase === "stopped" ? STOPPED_HANDOFF_DELAY_MS : COMPLETION_TOAST_DURATION_MS
		...
		if (!nextRunningProvider) {
			...
			if (parsed.updateId > 0 && parsed.providers && Object.keys(parsed.providers).length > 0) {
				displayRef.current = null
				toast.dismiss(PROVIDER_RUN_TOAST_ID)
			}
			return
...
export function ProviderRunToastManager() {
	...
	const [persistedRun, setPersistedRun] = useState<ActiveProviderRun | null>(null)
	...
	}, [activeRun?.jobId, jobStatusQuery.data?.status, pathname, router, searchParams, urlJobId])
