...
export async function updateWorkspaceDetails(args: {
	...
	const brandChanged = current.name.trim() !== nextName || current.domain.trim() !== nextDomain
...
export async function updateWorkspaceSchedule(args: {
	...
	await db.update(schema.workspaces).set({ schedule }).where(eq(schema.workspaces.id, workspaceId))
