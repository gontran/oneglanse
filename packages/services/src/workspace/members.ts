...
import type { JoinByCodeOrganization, JoinByCodeWorkspace } from "./_internal/types.js"
...
> {
	...
	if (rawCode.startsWith("workspace_")) {
		...
		if (parsed) {
			const orgRecord = await db.query.organization.findFirst({
				where: (org, { eq, or }) => or(eq(org.slug, parsed.orgCode), eq(org.id, parsed.orgCode)),
			})
			...
			const orgWorkspaces = await db
				...
				.where(
					and(eq(schema.workspaces.tenantId, orgRecord.id), isNull(schema.workspaces.deletedAt)),
				)
				.execute()
	...
	const existingWsMember = await db.query.workspaceMembers.findFirst({
		where: (wm, { eq, and, isNull }) =>
			and(eq(wm.workspaceId, joinedWorkspace.id), eq(wm.userId, userId), isNull(wm.deletedAt)),
	})
