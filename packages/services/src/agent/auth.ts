...
> = {
	...
	apple: {
		domainSuffixes: ["apple.com", "apple-cloudkit.com", "apple-dns.net", "icloud.com"],
	},
	facebook: {
...
function getUploadConfig(): {
	...
	if (!url || !token) {
		throw new Error("AGENT_AUTH_UPLOAD_URL and AGENT_AUTH_UPLOAD_TOKEN must be set together.")
	}
...
function compactStorageStateByDomainSuffixes(
	...
	const cookies = new Map<string, NonNullable<StorageState["cookies"]>[number]>()
	...
	for (const cookie of state.cookies ?? []) {
		...
		const normalizedCookie = {
			...
			...(isValidSameSite(cookie.sameSite) ? { sameSite: cookie.sameSite } : {}),
		}

		cookies.set(cookieStorageKey(normalizedCookie), normalizedCookie)
	}

	const origins = new Map<string, NonNullable<StorageState["origins"]>[number]>()
...
function compactStorageStateForProvider(provider: AuthProvider, state: StorageState): StorageState {
	return compactStorageStateByDomainSuffixes(state, AUTH_PROVIDER_CONFIG[provider].domainSuffixes)
}
...
async function readStorageStateFile(filePath: string): Promise<StorageState | null> {
	try {
...
async function getSessionUpdatedAt(provider: AuthProvider): Promise<string | null> {
	try {
...
async function readRuntimeProfileMetadata(
	...
	try {
		const raw = await readFile(getRuntimeProfileMetadataFile(provider), "utf-8")
...
function buildPersistedStatus(status: Partial<PersistedAuthStatus>): PersistedAuthStatus {
	return {
		connecting: false,
		lastUpdatedAt: null,
		syncedAt: null,
...
function getReusableIdentitySessionFile(provider: ReusableIdentityProvider): string {
	...
	const states = (
		await Promise.all(
			REUSABLE_IDENTITY_PROVIDERS.map(async (provider) => {
				...
				const preferredAuthProvider = getReusableIdentityConfig(provider).preferredAuthProvider
...
export async function readAuthLaunchSeedState(
	...
	const candidates = [reusableState, providerState].filter((s): s is StorageState =>
		hasUsableAuthState(s),
	)
...
export async function saveReusableIdentitySessions(
	...
	for (const provider of REUSABLE_IDENTITY_PROVIDERS) {
		...
		await writeFile(getReusableIdentitySessionFile(provider), JSON.stringify(nextState))
...
export async function readAuthSession(provider: AuthProvider): Promise<StorageState | null> {
...
export async function writeProviderAuthStatus(
	...
	await writeFile(statusFile, JSON.stringify(buildPersistedStatus(status), null, 2))
}
...
export async function resetProviderAuthData(provider: AuthProvider): Promise<void> {
	ensureAuthDirectories()
...
export async function uploadAuthSession(
	...
	if (!hasUsableAuthState(payloadState)) {
		throw new Error(`${AUTH_PROVIDER_DISPLAY[provider].displayName} session is missing or invalid.`)
	}
	...
	if (!response.ok) {
		throw new Error(`Auth session upload failed (${response.status}): ${await response.text()}`)
	}
...
export async function readProviderAuthStatuses(): Promise<ProviderAuthStatus[]> {
	...
	return Promise.all(
		AUTH_PROVIDER_LIST.map(async (provider) => {
			...
			return {
				provider,
				connected,
				connecting: Boolean(storedStatus?.connecting) && isProcessAlive(storedStatus?.launcherPid),
...
export function getAuthProviderForRuntimeProvider(provider: Provider): AuthProvider {
	return getAuthProviderForProvider(provider)
}

export async function isAuthProviderReady(provider: AuthProvider): Promise<boolean> {
...
export async function hasRuntimeProviderAuth(provider: Provider): Promise<boolean> {
...
export async function readAuthenticatedRuntimeProviders(
	...
	return authStates
		.filter((entry): entry is { provider: Provider; hasAuth: true } => entry.hasAuth)
...
export async function getRuntimeProfileSeedPlan(
	...
	const shouldBootstrap =
		...
		(!metadata || metadata.authStateHash !== authStateHash || !existsSync(userDataDir))
...
export async function getMissingRuntimeProviders(
	...
	const authenticatedProviders = await readAuthenticatedRuntimeProviders(providers)
	const authenticatedSet = new Set(authenticatedProviders)

	return [...new Set(providers)].filter((provider) => !authenticatedSet.has(provider))
}
