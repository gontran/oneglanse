...
export function buildProviderJobId(jobGroupId: string, provider: Provider): string {
	return `${jobGroupId}__${provider}`
}

export function buildProviderCancelKey(jobGroupId: string, provider: Provider): string {
...
async function enqueueProviderJob(payload: ProviderJobPayload): Promise<void> {
	...
	try {
		...
		throw new Error(`failed to enqueue ${payload.provider} provider job: ${toErrorMessage(err)}`)
	}
}
...
export async function enqueueProviderJobs(args: {
	...
	const { jobGroupId, prompts, userId, workspaceId, providers = PROVIDER_LIST } = args
...
export async function submitAgentJobGroup(args: {
	...
	const authenticatedProviders = await readAuthenticatedRuntimeProviders(allowedProviders)
	if (authenticatedProviders.length === 0) {
		const disconnectedProviders = await getMissingRuntimeProviders(allowedProviders)
		console.warn(
	...
	const progress = {
		...
		updateId: 0,
		providers: Object.fromEntries(authenticatedProviders.map((p) => [p, "pending"])) as Record<
			string,
			string
		>,
		results: Object.fromEntries(authenticatedProviders.map((p) => [p, 0])) as Record<
			string,
			number
		>,
		stats: {
...
export async function cancelProviderRun(args: {
	...
	await redis.publish(PROVIDER_STOP_CHANNEL, JSON.stringify({ jobGroupId, provider }))
	return { accepted: true }
}
