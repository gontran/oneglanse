const DEBUG_ENABLED = process.env.DEBUG_ENABLED === "true" || process.env.DEBUG_ENABLED === "1"
...
export const PROVIDER_LABELS: Record<string, string> = Object.fromEntries(
	Object.entries(RAW_PROVIDER_LABELS).map(([key, label]) => [key, centerLabel(label, 12)]),
)

function coloredPrefix(provider: string): string {
	const color = PROVIDER_COLORS[provider] ?? "\x1b[37m"
	const label = PROVIDER_LABELS[provider] ?? provider.toUpperCase().slice(0, 11).padEnd(11)
...
export const logger = {
	...
	warn: (...args: unknown[]) => {
		console.warn(`${ts()} ${contextPrefix()} ${YELLOW}⚠${R}`, ...formatArgs(args))
	},

	error: (...args: unknown[]) => {
		console.error(`${ts()} ${contextPrefix()} ${RED}✕${R}`, ...formatArgs(args))
	},

	success: (...args: unknown[]) => {
		console.log(`${ts()} ${contextPrefix()} ${GREEN}✓${R}`, ...formatArgs(args))
	},
...
export function createProviderLogger(provider: string): ProviderLogger {
	...
	return {
		...
		warn: (...args) => console.warn(`${ts()} ${prefix} ${YELLOW}⚠${R}`, ...formatArgs(args)),

		error: (...args) => console.error(`${ts()} ${prefix} ${RED}✕${R}`, ...formatArgs(args)),

		success: (...args) => console.log(`${ts()} ${prefix} ${GREEN}✓${R}`, ...formatArgs(args)),
