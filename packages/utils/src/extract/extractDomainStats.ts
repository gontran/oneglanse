...
export function extractDomainStats(responses: PromptResponse[]): {
	...
	const addDomain = (map: Map<string, DomainAccumulator>, url: string, hasSourceText: boolean) => {
	...
	for (const r of responses) {
		...
		for (const s of r.sources ?? []) {
			if (s?.url) {
				const hasText =
					typeof s.cited_text === "string" ? s.cited_text.trim().length > 0 : Boolean(s.cited_text)
	...
	const normalize = (map: Map<string, DomainAccumulator>): DomainStats[] => {
		const total = Array.from(map.values()).reduce((sum, d) => sum + d.totalOccurrences, 0)
		...
		return Array.from(map.entries()).map(([domain, stats]) => ({
			...
			usedPercentageAcrossAllDomains:
				total > 0 ? Number(((stats.totalOccurrences / total) * 100).toFixed(1)) : 0,
			avgSourcesPerDomain:
